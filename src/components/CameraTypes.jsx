...
class _CameraTypes extends React.Component {
  ...
  handleCreateCameraType() {
    var widthInMillimeters = parseInt(
      window.prompt("Width in millimeters ?"),
      10
    )
    if (!widthInMillimeters) {
      return
    }
    var heightInMillimeters = parseInt(
      window.prompt("Height in millimeters ?"),
      10
    )
    if (!heightInMillimeters) {
      return
    }
...
const CameraTypes = connect(mapStateToProps, mapDispatchToProps)(_CameraTypes)
