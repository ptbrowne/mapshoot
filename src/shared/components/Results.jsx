  ...
  render() {
    ...
    return cameras.length ? (
      <div className="panel-section">
        <h2>
          <i className="fa fa-picture-o" /> Snapshots
        </h2>
...
class _SelectedCamera extends React.Component {
  ...
  render() {
    ...
    return selectedCamera ? (
      <div className="panel-section">
        <h2>
          <i className="fa fa-camera" /> Current camera
        </h2>
        <img className="results__selected-camera" src={url} />
        <br />
        <p>
          <span className={rightZoom ? "bg--green" : "bg--red"}>
            {rightZoom
              ? "Camera and map at same zoom level"
              : "Camera and map at different zoom levels"}
          </span>
          <br />
          width: {selectedCamera.widthInMillimeters}mm
          <br />
        ...
        <div className="section__actions">
          ...
          <button
            ...
            onClick={onRemoveCamera.bind(null, selectedCamera)}
          >
            <i className="fa fa-times" /> Remove
          </button>
          <button
            onClick={onCreateCameraTypeFromCamera.bind(null, selectedCamera)}
            className="btn btn--green"
          >
            <i className="fa fa-camera-retro" /> Create camera type
          </button>
          <button
            onClick={onViewZoom.bind(null, selectedCamera)}
            className="btn btn--purple"
          >
            <i className="fa fa-search-plus" /> Center on camera
          </button>
          <button
            onClick={() => onSetZoom(selectedCamera, this.props.mapZoom)}
            className="btn btn--purple"
          >
